...
public:
  RuleGenerationImpl(std::shared_ptr<RuntimeImpl> runtime,
      ...
      : runtime_(std::move(runtime)), rules_(std::move(rules)),
        loaded_rule_count_(loaded_rule_count), source_count_(source_count) {}
...
private:
  ...
  std::shared_ptr<RuntimeImpl> runtime_;
...
public:
  ...
  compile(const std::vector<RuleSource>& sources) override {
    ...
    try {
      ...
      for (const RuleSource& source : sources) {
        ...
        if (loaded < 0) {
          return absl::InvalidArgumentError(
              absl::StrCat("failed to load rule source '", source.name,
      ...
      return absl::InternalError(
          absl::StrCat("libmodsecurity ruleset compilation threw: ", error.what()));
    } catch (...) {
      return absl::InternalError("libmodsecurity ruleset compilation threw");
    }
  }
...
absl::StatusOr<std::unique_ptr<Transaction>> RuleGenerationImpl::createTransaction() const {
  try {
    return std::unique_ptr<Transaction>(new TransactionImpl(std::make_unique<modsecurity::Transaction>(
        runtime_->native(), rules_.get(), nullptr)));
  } catch (const std::exception& error) {
    return absl::InternalError(
        absl::StrCat("libmodsecurity transaction creation threw: ", error.what()));
  } catch (...) {
    return absl::InternalError("libmodsecurity transaction creation threw");
  }
}
