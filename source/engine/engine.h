...
struct RuleSource {
  ...
  static RuleSource file(std::string filename) {
    return {Kind::File, std::move(filename), {}};
  }
...
public:
  ...
  virtual absl::Status processResponseHeaders(uint32_t status,
                                              absl::string_view http_version) = 0;
...
public:
  ...
  virtual absl::StatusOr<std::shared_ptr<const RuleGeneration>>
  compile(const std::vector<RuleSource>& sources) = 0;
};
